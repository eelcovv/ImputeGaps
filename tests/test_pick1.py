...
IMPUTATION_METHODS = {"pick1": ["float", "percentage", "dict", "bool", "int"]}
ID_KEY = "be_id"
SET_SEED = 1
...
def test_float():
    """
    Test voor var_type 'float' met imputatiemethode 'pick1'
    """
...
def test_percentage():
    """
    Test voor var_type 'percentage' met imputatiemethode 'pick1'
    """
    records_df = pd.DataFrame(
        [
            [1, 1, "A", "10", 1.1],
...
def test_bool():
    """
    Test voor var_type 'bool' met imputatiemethode 'pick1'
    """
    records_df = pd.DataFrame(
        [
            [1, 1, "A", "10", 1],
...
def test_met_filter():
    """
    Test voor var_type 'dict' met imputatiemethode 'pick1'
    """
    records_df = pd.DataFrame(
        [
            [1, 1, "A", "10", 1],
...
def test_met_impute_only():
    ...
    variables = {"telewerkers": {"type": "dict", "impute_only": "internet", "filter": "sbi"}}
